...
`T` の許容範囲（**MUST**）：
- 数値型（`i32` / `u32` / `f32` / `i64` / `u64` / `f64` / `boolean`）
- plain class（フィールドがすべて数値型または `_mNN` プリミティブで構成され、メソッド呼び出しを行わず、コンストラクタも持たないクラス）

言語仕様§14.3.1 および GPU IR 仕様§4 の `ptr<T>` と同一の T 許容範囲を採用する（**MUST**）。これにより CPU 側 `Ptr<T>` と GPU 側 `ptr<T>` は型表現が共通化され、`GpuArgs.pushBufferMut`（§8.3.2）等の境界 API が同一概念を扱える。

ヒープ上のレイアウトは `__builtin_sizeof<T>` が返すサイズに従い決定される。plain class の場合はフィールドが宣言順にパッキングされる（**MUST**、アラインメントは実装依存）。
...
ロックフリーアルゴリズムの実装に使用する。`ptr` はバイトアドレス（言語仕様§3.2 と同じ単位）。32bit アトミックは 4 バイト境界、64bit アトミックは 8 バイト境界にアライメントされている必要がある（**MUST**、違反時の挙動は **IMPLEMENTATION-DEFINED**）。
...
**64bit 操作**（`ptr` は 8 バイト境界にアライメントされている必要がある）
