...
> **注意:** 所有権・借用システムの拡張導入に伴い、`this` の型はメソッドの `mut` 修飾子の有無によって `&クラス名`（不変参照）または `&mut クラス名`（可変参照）として扱われるよう変更された。詳細な挙動と制約については「[mozaicScript 所有権・借用システム拡張追加仕様書](mozaicScript-ownership-spec.md)」を参照すること。

| 用途 | 可否 |
|------|------|
| メンバへのアクセス（`this.x` / `this.f()`） | ✅ 許可 |
| 返り値にする（`return this;`） | ❌ 禁止（※借用ルールによるダングリング参照防止のため） |
